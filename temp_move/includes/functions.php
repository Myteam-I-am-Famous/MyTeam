...
function createArticle($title, $content, $imagePath = null, $author)
{
    global $dbh;
...
function getDataFrom($table, $orderBy = '')
{

    global $dbh;

    if ($orderBy != '')
        $q = 'SELECT * FROM ' . $table . ' ORDER BY ' . $orderBy . ';';
    else
        $q = 'SELECT * FROM ' . $table . ';';
    $stmt = $dbh->prepare($q);
...
function getCountFrom($table, $condition = '', $value = 1)
{
    global $dbh;

    if ($condition != '') {
        $q = 'SELECT COUNT(*) FROM ' . $table . ' WHERE ' . $condition . ' = :value;';
        $stmt = $dbh->prepare($q);
        $status = $stmt->execute(
            array(
                'value' => $value
...
function updateUserStatus($uid, $value)
{
    global $dbh;

    $q = 'UPDATE users SET status = :value WHERE id = :uid;';
    $stmt = $dbh->prepare($q);
    $status = $stmt->execute(
        array(
            'value' => $value,
